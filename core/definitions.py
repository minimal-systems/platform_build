...
def include_depfile(depfile: str, main_target: str, all_targets: dict) -> None:
    """
    Include a dependency file generated by the compiler for the main build target.

    Args:
        depfile (str): Path to the .P dependency file.
        main_target (str): The main build target.
        all_targets (dict): Dictionary to store the targets and their dependencies.

    Returns:
        None: Updates the `all_targets` dictionary in place.
    """
    if not main_target or not depfile:
        raise ValueError("Both depfile and main_target must be provided.")

    # Add the dependency file to the main target's attributes
    if main_target not in all_targets:
        all_targets[main_target] = {}
    all_targets[main_target]["KATI_DEPFILE"] = depfile
