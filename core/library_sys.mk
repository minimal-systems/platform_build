...
RECOVERY_CFLAGS= -DRECOVERY_LINKER -DRECOVERY_MODULE -DRECOVERY_LIBRARY -DRECOVERY_INIT -DRECOVERY_LIBRARY
...
INSTALL_EXEC_PATH= $(TARGET_VENDOR_OUT)/lib
endif
else
ifeq ($(OVERRIDE_INSTALL_PATH),true)
INSTALL_EXEC_PATH=$(INSTALL_PATH)
else
INSTALL_EXEC_PATH= $(TARGET_SYSTEM_OUT)/lib
endif
endif
...
INSTALLED_LIBRARY_TARGET=$(INSTALL_EXEC_PATH)/$(LOCAL_MODULE).so
$(INSTALLED_LIBRARY_TARGET):
	...
	@$(CC) -o $(INSTALLED_LIBRARY_TARGET)  $(LOCAL_OBJS) -Wl,--as-needed $(LD_FLAGS) -Wl,--no-undefined
	@echo -e $(BOLD)"\e[1A\e[KInstall:$@"${CL_RST}
	@strip $(STRIP_FLAGS) $(OBJ)/LINKED/$(LOCAL_MODULE)
	@strip $(STRIP_FLAGS) $(INSTALLED_LIBRARY_TARGET)
...
$(INSTALLED_RECOVERY_LIBRARY_TARGET):
	...
	@$(CC) -o $(INSTALLED_RECOVERY_LIBRARY_TARGET) $(RECOVERY_LOCAL_OBJS)  -Wl,--as-needed $(LD_FLAGS) -Wl,--no-undefined
	@echo -e $(BOLD)"\e[1A\e[KInstall:$@"${CL_RST}
	@strip $(STRIP_FLAGS) $(RECOVERY_OBJ)/LINKED/$(LOCAL_MODULE)
	@strip $(STRIP_FLAGS) $(INSTALLED_RECOVERY_LIBRARY_TARGET)
